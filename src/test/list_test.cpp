...
TEST(ListContainer, test_compare_iters) {
  containers::list<int> list{1, 2, 3};
  containers::list<int>::iterator it = list.begin(), it2 = list.begin();
  EXPECT_TRUE(it == it2);
  EXPECT_FALSE(it != it2);
  ++it2;
  EXPECT_TRUE(it != it2);
...
TEST(ListContainer, test_swap) {
  list<int> list1{1, 2, 3}, list2{4, 5};
  EXPECT_EQ(list1.size(), (size_t)3);
...
TEST(ListContainer, test_reverse) {
  list<int> list1{1, 2, 3}, list2{4, 5}, list3{6};
  list1.reverse();
  list2.reverse();
  list3.reverse();
...
TEST(ListContainer, test_sort) {
  list<int> list1{1, 24, 3}, list2{5, 4}, list3{6},
...
TEST(ListContainer, test_begin_plus_minus) {
  list<int> list = {1, 2, 3};
  containers::list<int>::iterator it = list.begin();
  EXPECT_EQ(*it, 1);
  ++it;
  EXPECT_EQ(*it, 2);
  ++it;
...
TEST(ListContainer, test_end_plus_minus) {
  list<int> list = {1, 2, 3};
  containers::list<int>::iterator it = list.end();
  EXPECT_EQ(*it, 3);
  --it;
  EXPECT_EQ(*it, 2);
  ++it;
...
TEST(ListContainer, test_comparison) {
  list<int> list = {6, 5, 6, 15};
  containers::list<int>::iterator it = list.begin(), it2 = list.end();
  EXPECT_TRUE(*it <= *it2);
  EXPECT_TRUE(*it < *it2);
  EXPECT_FALSE(*it >= *it2);
  EXPECT_FALSE(*it > *it2);
...
TEST(ListContainer, test_merge) {
  list<int> list = {1, 2, 2, 3, 3, 3, 3, 4, 5, 4, 6, 1},
            list2 = {14, 15, 12, 13, 30, 3};
  ASSERT_EQ(list.size(), (size_t)12);
  list.merge(list2);
  ASSERT_EQ(list.size(), (size_t)18);
...
TEST(ListContainer, test_splice) {
  list<int> list = {1, 2, 6, 7}, list2 = {3, 4, 5};
  containers::list<int>::const_iterator it = list.begin();
  list.splice(it, list2);
  containers::list<int>::iterator it_check = list.begin();
  ...
  containers::list<int> list3 = {1, 2, 6, 7}, list4 = {3, 4, 5};
  it = list3.begin();
  ++it;
  ++it;
