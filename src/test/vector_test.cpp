...
TEST(VectorCapacity, reserve_capacity_out_of_range) {
  vector<int> vec;
  EXPECT_THROW(
      vec.reserve(std::numeric_limits<vector<int>::size_type>::max() + 1),
      std::out_of_range);
}
