...
TEST(ArrayCapacity, max_size) {
  ...
  typename array<int>::size_type expected_max_size =
      std::numeric_limits<typename array<int>::size_type>::max();

  // Вызываем метод max_size() и сохраняем результат
  typename array<int>::size_type actual_max_size = own_array.max_size();
