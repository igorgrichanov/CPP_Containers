#ifndef STACKCONTAINER_TPP_
#define STACKCONTAINER_TPP_
...
void stack<T>::push(typename SqContainer<T>::const_reference value) {
  typename containers::list<T>::Node* node =
      new typename containers::list<T>::Node(value);
  node->prev = nullptr;
...
void stack<T>::pop() {
  if (list.getHead()) {
    typename containers::list<T>::Node* head_temp = list.getHead();
